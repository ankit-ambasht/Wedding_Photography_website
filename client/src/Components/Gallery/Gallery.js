...
const Gallery = () => {
  ...
  useEffect(() => {
    axios
      ...
      // )
      .get(
        `https://graph.instagram.com/me/media?fields=id,caption,media_url,timestamp,like,media_type&expires_in:5183944&client_secret=${process.env.RAECT_APP_ACCESS_KEY}&access_token=${process.env.REACT_APP_TOKEN_KEY}`
      )
      .then((res) => {
        setLoading(true);
